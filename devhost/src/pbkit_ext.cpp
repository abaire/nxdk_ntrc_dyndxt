...
void set_depth_stencil_buffer_region(uint32_t depth_buffer_format,
  ...
  p = pb_push1(p, NV097_SET_CLEAR_RECT_HORIZONTAL,
               ((right - 1) << 16) | (left & 0xFFFF));
  p = pb_push1(p, NV097_SET_CLEAR_RECT_VERTICAL,
               ((bottom - 1) << 16) | (top & 0xFFFF));
  ...
  switch (depth_buffer_format) {
    ...
    case NV097_SET_SURFACE_FORMAT_ZETA_Z24S8:
      p = pb_push1(p, NV097_SET_ZSTENCIL_CLEAR_VALUE,
                   ((depth_value & 0x00FFFFFF) << 8) | stencil_value);
      break;
...
float z24_to_float(uint32_t val) {
  ...
  // XBOX 24 bit format is e8m16, convert it to a 32-bit float by shifting the
  // exponent portion to 23:30.
...
//   p = pb_push2(p, NV20_TCL_PRIMITIVE_3D_PARAMETER_A, NV_PRAMIN +
//   (context->Inst << 4) + 0x08, dma_addr);
//
//   // calls subprogID PB_SETOUTER: does VIDEOREG(addr)=data
//   p = pb_push1(p, NV20_TCL_PRIMITIVE_3D_FIRE_INTERRUPT, PB_SETOUTER);
//   p = pb_push2(p, NV20_TCL_PRIMITIVE_3D_PARAMETER_A, NV_PRAMIN +
//   (context->Inst << 4) + 0x0C, dma_addr); p = pb_push1(p,
//   NV20_TCL_PRIMITIVE_3D_FIRE_INTERRUPT, PB_SETOUTER); p = pb_push2(p,
//   NV20_TCL_PRIMITIVE_3D_PARAMETER_A, NV_PRAMIN + (context->Inst << 4) + 0x00,
//   dma_flags); p = pb_push1(p, NV20_TCL_PRIMITIVE_3D_FIRE_INTERRUPT,
//   PB_SETOUTER); p = pb_push2(p, NV20_TCL_PRIMITIVE_3D_PARAMETER_A, NV_PRAMIN
//   + (context->Inst << 4) + 0x04, limit); p = pb_push1(p,
//   NV20_TCL_PRIMITIVE_3D_FIRE_INTERRUPT, PB_SETOUTER);
//
//   pb_end(p);
// }
//
...
// {
//   auto p = pb_begin();
//   p = pb_push1_to(subchannel, p, NV20_TCL_PRIMITIVE_SET_MAIN_OBJECT,
//   context->ChannelID); pb_end(p);
// }

void *pb_agp_access(void *fb_memory_pointer) {
  return reinterpret_cast<void *>(
      reinterpret_cast<uint32_t>(fb_memory_pointer) | AGP_MEMORY_REMAP);
}
