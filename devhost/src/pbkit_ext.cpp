...
void pb_fetch_pgraph_registers(uint8_t *registers) {
  // See
  // https://github.com/XboxDev/nv2a-trace/blob/65bdd2369a5b216cfc47c9545f870c49d118276b/Trace.py#L32

  while (pb_busy()) {
    /* Wait for any pending pgraph commands to be flushed */
  }

  constexpr uint32_t kPGRAPHRegisterEnd =
      PGRAPH_REGISTER_BASE + PGRAPH_REGISTER_ARRAY_SIZE;
...
void pb_diff_registers(const uint8_t *a, const uint8_t *b,
  ...
  for (auto i = 0; i < PGRAPH_REGISTER_ARRAY_SIZE / 4;
       ++i, ++old_val, ++new_val) {
    if (*old_val != *new_val) {
