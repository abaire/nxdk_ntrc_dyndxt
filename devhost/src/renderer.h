#ifndef NTRC_DYNDXT_DEVHOST_SRC_RENDERER_H_
#define NTRC_DYNDXT_DEVHOST_SRC_RENDERER_H_
...
class Renderer {
 public:
  ...
  static constexpr uint32_t kDefaultVertexFields =
  ...
  struct ColorInput : public CombinerInput {
    explicit ColorInput(CombinerSource s,
                        CombinerMapping m = MAP_UNSIGNED_IDENTITY)
        : CombinerInput() {
      source = s;
      alpha = false;
      mapping = m;
  ...
  struct AlphaInput : public CombinerInput {
    explicit AlphaInput(CombinerSource s,
                        CombinerMapping m = MAP_UNSIGNED_IDENTITY)
        : CombinerInput() {
      source = s;
      alpha = true;
 ...
 public:
  Renderer(uint32_t framebuffer_width, uint32_t framebuffer_height,
           uint32_t max_texture_width, uint32_t max_texture_height,
           uint32_t max_texture_depth = 4);
  ~Renderer();
  ...
  int SetRawTexture(const uint8_t *source, uint32_t width, uint32_t height,
                    uint32_t depth, uint32_t pitch, uint32_t bytes_per_pixel,
                    uint32_t stage = 0);
  ...
  // width and height are treated differently depending on whether swizzle is
  // enabled or not. swizzle = true
  //     width and height must be a power of two
  // swizzle = false
  //     width and height may be arbitrary positive values and will be used to
  //     set the clip dimensions
  //
  // Does not take effect until CommitSurfaceFormat is called.
  void SetSurfaceFormat(SurfaceColorFormat color_format,
                        SurfaceZetaFormat depth_format, uint32_t width,
                        uint32_t height, bool swizzle = false,
                        uint32_t clip_x = 0, uint32_t clip_y = 0,
  ...
  void SetSurfaceFormatImmediate(SurfaceColorFormat color_format,
                                 SurfaceZetaFormat depth_format, uint32_t width,
                                 uint32_t height, bool swizzle = false,
                                 uint32_t clip_x = 0, uint32_t clip_y = 0,
  ...
  inline float GetFramebufferWidthF() const {
    return static_cast<float>(framebuffer_width_);
  }
  inline float GetFramebufferHeightF() const {
    return static_cast<float>(framebuffer_height_);
  }
  ...
  void Clear(uint32_t argb = 0xFF000000, uint32_t depth_value = 0xFFFFFFFF,
             uint8_t stencil_value = 0x00) const;
  void ClearDepthStencilRegion(uint32_t depth_value, uint8_t stencil_value,
                               uint32_t left = 0, uint32_t top = 0,
                               uint32_t width = 0, uint32_t height = 0) const;
  void ClearColorRegion(uint32_t argb, uint32_t left = 0, uint32_t top = 0,
                        uint32_t width = 0, uint32_t height = 0) const;
  static void EraseText();

  // Note: A number of states are expected to be set before this method is
  // called. E.g., texture stages, shader states This is not an exhaustive list
  // and is not necessarily up to date. Prefer to call this just before
  // initiating draw and be suspect of order dependence if you see results that
  // seem to indicate that settings are being ignored.
  void PrepareDraw(uint32_t argb = 0xFF000000,
                   uint32_t depth_value = 0xFFFFFFFF,
                   uint8_t stencil_value = 0x00);

  void DrawArrays(uint32_t enabled_vertex_fields = kDefaultVertexFields,
                  DrawPrimitive primitive = PRIMITIVE_TRIANGLES);
  void DrawInlineBuffer(uint32_t enabled_vertex_fields = kDefaultVertexFields,
                        DrawPrimitive primitive = PRIMITIVE_TRIANGLES);

  // Sends vertices as an interleaved array of vertex fields. E.g.,
  // [POS_0,DIFFUSE_0,POS_1,DIFFUSE_1,...]
  void DrawInlineArray(uint32_t enabled_vertex_fields = kDefaultVertexFields,
                       DrawPrimitive primitive = PRIMITIVE_TRIANGLES);

  // Sends vertices via an index array. Index values must be < 0xFFFF and are
  // sent two per command.
  void DrawInlineElements16(
      const std::vector<uint32_t> &indices,
      uint32_t enabled_vertex_fields = kDefaultVertexFields,
      DrawPrimitive primitive = PRIMITIVE_TRIANGLES);

  // Sends vertices via an index array. Index values are unsigned integers.
  void DrawInlineElements32(
      const std::vector<uint32_t> &indices,
      uint32_t enabled_vertex_fields = kDefaultVertexFields,
      DrawPrimitive primitive = PRIMITIVE_TRIANGLES);
  ...
  static void BuildD3DModelViewMatrix(MATRIX matrix, const VECTOR eye,
                                      const VECTOR at, const VECTOR up);

  // Gets a D3D-style matrix suitable for a projection + viewport transform.
  void BuildD3DProjectionViewportMatrix(MATRIX result, float fov, float z_near,
                                        float z_far) const;

  // Gets a reasonable default model view matrix (camera at z=-7.0f looking at
  // the origin)
  static void BuildDefaultXDKModelViewMatrix(MATRIX matrix);
  // Gets a reasonable default projection matrix (fov = PI/4, near = 1, far =
  // 200)
  void BuildDefaultXDKProjectionMatrix(MATRIX matrix) const;

  const float *GetFixedFunctionInverseCompositeMatrix() const {
    return fixed_function_inverse_composite_matrix_;
  }

  // Set up the viewport and fixed function pipeline matrices to match a default
  // XDK project.
  void SetXDKDefaultViewportAndFixedFunctionMatrices();

  // Set up the viewport and fixed function pipeline matrices to match the nxdk
  // settings.
  void SetDefaultViewportAndFixedFunctionMatrices();

  // Projects the given point (on the CPU), placing the resulting screen
  // coordinates into `result`.
  ...
  void UnprojectPoint(VECTOR result, const VECTOR screen_point,
                      float world_z) const;

  static void SetWindowClipExclusive(bool exclusive);
  static void SetWindowClip(uint32_t right, uint32_t bottom, uint32_t left = 0,
                            uint32_t top = 0, uint32_t region = 0);
  ...
  inline const float *GetFixedFunctionModelViewMatrix() const {
    return fixed_function_model_view_matrix_;
  }
  inline const float *GetFixedFunctionProjectionMatrix() const {
    return fixed_function_projection_matrix_;
  }

  // Start the process of rendering an inline-defined primitive (specified via
  // SetXXXX methods below). Note that End() must be called to trigger
  // rendering, and that SetVertex() triggers the creation of a vertex.
  ...
  void SetColorMask(
      uint32_t mask = NV097_SET_COLOR_MASK_BLUE_WRITE_ENABLE |
  ...
  void SetBlend(bool enable = true,
                ...
                uint32_t dfactor =
                    NV097_SET_BLEND_FUNC_DFACTOR_V_ONE_MINUS_SRC_ALPHA) const;
  ...
  void SetCombinerControl(int num_combiners = 1, bool same_factor0 = false,
                          bool same_factor1 = false,
                          bool mux_msb = false) const;

  void SetInputColorCombiner(int combiner, CombinerInput a,
                             CombinerInput b = ZeroInput(),
                             CombinerInput c = ZeroInput(),
                             CombinerInput d = ZeroInput()) const {
    SetInputColorCombiner(combiner, a.source, a.alpha, a.mapping, b.source,
                          b.alpha, b.mapping, c.source, c.alpha, c.mapping,
                          d.source, d.alpha, d.mapping);
  }
  void SetInputColorCombiner(
      int combiner, CombinerSource a_source = SRC_ZERO, bool a_alpha = false,
      CombinerMapping a_mapping = MAP_UNSIGNED_IDENTITY,
      CombinerSource b_source = SRC_ZERO, bool b_alpha = false,
      ...
      CombinerSource d_source = SRC_ZERO, bool d_alpha = false,
      CombinerMapping d_mapping = MAP_UNSIGNED_IDENTITY) const;
  void ClearInputColorCombiner(int combiner) const;
  void ClearInputColorCombiners() const;

  void SetInputAlphaCombiner(int combiner, CombinerInput a,
                             CombinerInput b = ZeroInput(),
                             CombinerInput c = ZeroInput(),
                             CombinerInput d = ZeroInput()) const {
    SetInputAlphaCombiner(combiner, a.source, a.alpha, a.mapping, b.source,
                          b.alpha, b.mapping, c.source, c.alpha, c.mapping,
                          d.source, d.alpha, d.mapping);
  }
  void SetInputAlphaCombiner(
      int combiner, CombinerSource a_source = SRC_ZERO, bool a_alpha = false,
      CombinerMapping a_mapping = MAP_UNSIGNED_IDENTITY,
      CombinerSource b_source = SRC_ZERO, bool b_alpha = false,
      CombinerMapping b_mapping = MAP_UNSIGNED_IDENTITY,
  ...
  void SetOutputColorCombiner(
      int combiner, CombinerDest ab_dst = DST_DISCARD,
      CombinerDest cd_dst = DST_DISCARD, CombinerDest sum_dst = DST_DISCARD,
      bool ab_dot_product = false, bool cd_dot_product = false,
      CombinerSumMuxMode sum_or_mux = SM_SUM, CombinerOutOp op = OP_IDENTITY,
      bool alpha_from_ab_blue = false, bool alpha_from_cd_blue = false) const;
  void ClearOutputColorCombiner(int combiner) const;
  void ClearOutputColorCombiners() const;

  void SetOutputAlphaCombiner(int combiner, CombinerDest ab_dst = DST_DISCARD,
  ...
  void SetFinalCombiner0Just(CombinerSource d_source, bool d_alpha = false,
                             bool d_invert = false) const {
    SetFinalCombiner0(SRC_ZERO, false, false, SRC_ZERO, false, false, SRC_ZERO,
                      false, false, d_source, d_alpha, d_invert);
  }
  void SetFinalCombiner0(CombinerSource a_source = SRC_ZERO,
                         bool a_alpha = false, bool a_invert = false,
                         CombinerSource b_source = SRC_ZERO,
                         bool b_alpha = false, bool b_invert = false,
                         CombinerSource c_source = SRC_ZERO,
                         bool c_alpha = false, bool c_invert = false,
                         CombinerSource d_source = SRC_ZERO,
                         bool d_alpha = false, bool d_invert = false) const;

  void SetFinalCombiner1Just(CombinerSource g_source, bool g_alpha = false,
                             bool g_invert = false) const {
    SetFinalCombiner1(SRC_ZERO, false, false, SRC_ZERO, false, false, g_source,
                      g_alpha, g_invert);
  }
  void SetFinalCombiner1(CombinerSource e_source = SRC_ZERO,
                         bool e_alpha = false, bool e_invert = false,
                         CombinerSource f_source = SRC_ZERO,
                         bool f_alpha = false, bool f_invert = false,
                         CombinerSource g_source = SRC_ZERO,
                         bool g_alpha = false, bool g_invert = false,
  ...
  void SetCombinerFactorC0(int combiner, float red, float green, float blue,
                           float alpha) const;
  void SetCombinerFactorC1(int combiner, uint32_t value) const;
  void SetCombinerFactorC1(int combiner, float red, float green, float blue,
                           float alpha) const;

  void SetFinalCombinerFactorC0(uint32_t value) const;
  void SetFinalCombinerFactorC0(float red, float green, float blue,
                                float alpha) const;
  void SetFinalCombinerFactorC1(uint32_t value) const;
  void SetFinalCombinerFactorC1(float red, float green, float blue,
                                float alpha) const;

  // Sets the type of texture sampling for each texture.
  //
  // If you have a totally blank texture, double check that this is set to
  // something other than STAGE_NONE.
  void SetShaderStageProgram(ShaderStageProgram stage_0,
                             ShaderStageProgram stage_1 = STAGE_NONE,
                             ShaderStageProgram stage_2 = STAGE_NONE,
                             ShaderStageProgram stage_3 = STAGE_NONE) const;
  // Sets the input for shader stage 2 and 3. The value is the 0 based index of
  // the stage whose output should be linked. E.g., to have stage2 use stage1's
  // input and stage3 use stage2's the params would be (1, 2).
  void SetShaderStageInput(uint32_t stage_2_input = 0,
                           uint32_t stage_3_input = 0) const;

  void SetVertexBufferAttributes(uint32_t enabled_fields);

  void SetupControl0(bool enable_stencil_write = true) const;

  // Commit any changes to texture stages (called automatically in PrepareDraw
  // but may be useful to call more frequently in scenes with multiple draws per
  // clear)
  void SetupTextureStages() const;

  // Returns the maximum possible value that can be stored in the depth surface
  // for the given mode.
  static float MaxDepthBufferValue(uint32_t depth_buffer_format,
                                   bool float_mode);
  ...
  // Rounds the given integer in the same way as nv2a hardware (only remainders
  // >= 9/16th are rounded up).
  static float NV2ARound(float input);

 private:
  // Update matrices when the depth buffer format changes.
  void HandleDepthBufferFormatChange();
  uint32_t MakeInputCombiner(CombinerSource a_source, bool a_alpha,
                             CombinerMapping a_mapping, CombinerSource b_source,
                             bool b_alpha, CombinerMapping b_mapping,
                             CombinerSource c_source, bool c_alpha,
                             CombinerMapping c_mapping, CombinerSource d_source,
                             bool d_alpha, CombinerMapping d_mapping) const;
  uint32_t MakeOutputCombiner(CombinerDest ab_dst, CombinerDest cd_dst,
                              CombinerDest sum_dst, bool ab_dot_product,
                              bool cd_dot_product,
 ...
 private:
  ...
  FixedFunctionMatrixSetting fixed_function_matrix_mode_{
      MATRIX_MODE_DEFAULT_NXDK};
...
#endif  // NTRC_DYNDXT_DEVHOST_SRC_RENDERER_H_
