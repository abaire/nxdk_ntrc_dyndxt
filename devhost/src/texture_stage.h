...
class TextureStage {
 ...
 public:
  ...
  void SetQWrap(bool cylinder_wrap = false) {
    cylinder_wrap_[3] = cylinder_wrap;
  }
  ...
  // Determines whether texels along the border retrieve their color from
  // border_color_ or the texture.
  void SetBorderFromColor(bool val = true) { border_source_color_ = val; }

  void SetBumpEnv(float x, float y, float z, float w, float scale,
                  float offset = 0.0f) {
  ...
  void SetFilter(uint32_t lod_bias = 0, ConvolutionKernel kernel = K_QUINCUNX,
                 MinFilter min = MIN_BOX_LOD0, MagFilter mag = MAG_BOX_LOD0,
                 bool signed_alpha = false, bool signed_red = false,
                 bool signed_green = false, bool signed_blue = false);

  [[nodiscard]] bool IsSwizzled() const { return format_.xbox_swizzled; }
  [[nodiscard]] bool IsLinear() const { return format_.xbox_linear; }
  [[nodiscard]] bool RequiresColorspaceConversion() const;

  void SetTextureMatrixEnable(bool enabled) {
    texture_matrix_enable_ = enabled;
  }
  [[nodiscard]] bool GetTextureMatrixEnable() const {
    return texture_matrix_enable_;
  }
  void SetTextureMatrix(const float *matrix) {
    memcpy(texture_matrix_, matrix, sizeof(texture_matrix_));
  }
  [[nodiscard]] const float *GetTextureMatrix() const {
    return texture_matrix_;
  }
  float *GetTextureMatrix() { return texture_matrix_; }

  [[nodiscard]] TexGen GetTexgenS() const { return texgen_s_; }
  [[nodiscard]] TexGen GetTexgenT() const { return texgen_t_; }
  [[nodiscard]] TexGen GetTexgenR() const { return texgen_r_; }
  [[nodiscard]] TexGen GetTexgenQ() const { return texgen_q_; }
  ...
  [[nodiscard]] uint32_t GetMipMapLevels() const { return mipmap_levels_; }
 ...
 private:
  ...
  [[nodiscard]] uint32_t GetTextureOffset() const {
    return texture_memory_offset_;
  }
  void SetTextureOffset(uint32_t offset) { texture_memory_offset_ = offset; }
  [[nodiscard]] uint32_t GetPaletteOffset() const {
    return palette_memory_offset_;
  }
  ...
  int SetPalette(const uint32_t *palette, uint32_t length,
                 uint8_t *memory_base);
 ...
 private:
  ...
  WrapMode wrap_modes_[3]{WRAP_CLAMP_TO_EDGE, WRAP_CLAMP_TO_EDGE,
                          WRAP_CLAMP_TO_EDGE};
