...
class VertexShaderProgram {
 public:
  ...
  // Note: It is the caller's responsibility to ensure that the shader array
  // remains valid for the lifetime of this instance.
  void SetShaderOverride(const uint32_t *shader, uint32_t shader_size,
                         int32_t uniform_start_offset = 0) {
  ...
  void SetUniformF(uint32_t slot, float x, float y = 0.0f, float z = 0.0f,
                   float w = 0.0f);
  void SetUniformI(uint32_t slot, uint32_t x, uint32_t y = 0, uint32_t z = 0,
                   uint32_t w = 0);
 ...
 protected:
  ...
  void SetTransformConstantBlock(uint32_t slot, const uint32_t *values,
                                 uint32_t num_slots);
  ...
  void SetBaseUniformF(uint32_t slot, float x, float y = 0.0f, float z = 0.0f,
                       float w = 0.0f);
  void SetBaseUniformI(uint32_t slot, uint32_t x, uint32_t y = 0,
                       uint32_t z = 0, uint32_t w = 0);

  void SetUniformBlock(uint32_t slot, const uint32_t *values,
                       uint32_t num_slots);
