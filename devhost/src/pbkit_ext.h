...
// commands. E.g., NV097_INLINE_ARRAY (0x00001818) expects a single parameter
// which is added to the inline array being built up in graphics memory. Rather
// than invoke pb_push1 many times over and waste time/space re-sending the
// NV097_INLINE_ARRAY command, it is desirable to send multiple components
// (e.g., all 3 coordinates for a vertex) in a single push. Setting bit 30
// (i.e., NV2A_SUPPRESS_COMMAND_INCREMENT(NV097_INLINE_ARRAY)) instructs the
// pushbuffer to treat all the parameters in the command packet as part of the
// NV097_INLINE_ARRAY command. Failure to set bit 30 would utilize the default
// behavior, so the first parameter would go to NV097_INLINE_ARRAY, but the next
// would go to NV097_SET_EYE_VECTOR (0x0000181C), which itself will expect 3
// parameters and will at best lead to unexpected behavior and potentially an
// exception being raised by the hardware depending on how many parameters in
// total are being sent.
#define NV2A_SUPPRESS_COMMAND_INCREMENT(cmd) (0x40000000 | (cmd))

void set_depth_stencil_buffer_region(uint32_t depth_buffer_format,
...
void pb_set_dma_address(const struct s_CtxDma* context, const void* address,
                        uint32_t limit);
...
// Pushes a 4 column x 3 row matrix (used to push the inverse matrix which
// intentionally omits the 4th row and is not transposed).
...
uint32_t* pb_push3f(uint32_t* p, DWORD command, float param1, float param2,
                    float param3);
